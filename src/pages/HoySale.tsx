...
import { getHolyWeekDay, daysUntilPalmSunday } from '@/utils/dateUtils'
...
export default function HoySale() {
  ...
  const daysToPalm = daysUntilPalmSunday(today)
  const isPreSemana = daysToPalm > 0
  const isPostSemana = daysToPalm < -7
  ...
  return (
    <>
      <SeoHead
        title="Hoy Sale... · Procesiones de Hoy"
        description={todayLabel ? `Descubre las procesiones de hoy ${todayLabel} en Semana Santa de Écija 2025.` : "Consulta qué hermandades salen hoy en Écija: horarios, itinerarios y más."}
        url="/hoy-sale"
      />
      <div className="container py-4">
        ...
        <div className="mb-4 p-4 rounded-3 shadow-sm" style={{
          ...
          <p className="mb-0 opacity-90" style={{ fontSize: '0.95rem' }}>
            {todayLabel ? (
              ...
              <>Faltan <strong>{daysToPalm} días</strong> para el Domingo de Ramos</>
            ) : isPostSemana ? (
            ...
            <div className="card-body text-center py-5">
              ...
              <p className="text-muted mb-0" style={{ maxWidth: '400px', margin: '0 auto' }}>
                {todayLabel
                  ...
                  : isPreSemana
                    ? `La Semana Santa de Écija ${year} comenzará en ${daysToPalm} días. ¡Prepárate!`
                    : isPostSemana
                      ? `La Semana Santa de Écija ${year} ha concluido. ¡Hasta el año que viene!`
