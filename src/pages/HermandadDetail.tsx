...
import NazarenoIcon from '@/components/NazarenoIcon'
import { parseNazarenoColors } from '@/utils/nazarenoColors'
...
export default function HermandadDetail() {
  ...
  const nazarenoColors = useMemo(() => {
    if (!hermandad) return { tunic: '#F2F2F2', hood: '#5C2D91', cape: '#F2F2F2' }
    const primero = hermandad.pasos?.[0]
    return parseNazarenoColors(hermandad.colors, primero?.tunica_nazarenos)
  }, [hermandad])
  ...
  return (
    <>
      <SeoHead
        title={`${hermandad.name} · ${hermandad.day}`}
        description={`Horarios, itinerario y detalles de la ${hermandad.name} para el ${hermandad.day} en la Semana Santa de Écija 2025.`}
        image={hermandad.mainImage}
        url={`/hermandades/${hermandad.id}`}
        type="article"
      />
