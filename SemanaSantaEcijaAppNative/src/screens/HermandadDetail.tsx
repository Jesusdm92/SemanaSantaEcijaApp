...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, Image, Platform, Linking, Alert } from 'react-native'
...
export default function HermandadDetail() {
  ...
  const headerSource = useMemo(() => {
    ...
    if (isRemote) return { uri: imageIdentifier }
  ...
  const colorBadges = useMemo(() => {
    ...
    return Array.from(new Set(tokens.map(t => map[t])))
  }, [hermandad.colors])
  ...
  // ─── Sección de Horarios reutilizable ─────────────────────────────────────────
  ...
  const InfoSection = (hermandad.tituloCompleto || hermandad.anoFundacion || hermandad.iglesia || hermandad.fecha || hermandad.webOficial) ? (
    <SectionCard title="Información" icon="information-circle-outline">
      ...
      {hermandad.webOficial && hermandad.webOficial !== 'Información no encontrada' && (
        <TouchableOpacity
          ...
          onPress={async () => {
            try {
              const url = hermandad.webOficial?.startsWith('http') ? hermandad.webOficial : `https://${hermandad.webOficial}`
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* TABLET: header de gradiente con nombre — sin imagen */}
        {isTablet ? (
          <View style={styles.tabletHeader}>
            <LinearGradient
              colors={[colors.primary, '#3d0070', colors.primary]}
              start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
              style={StyleSheet.absoluteFill}
            />
            <View style={styles.tabletHeaderTopBar}>
            ...
            <View style={styles.tabletHeaderContent}>
              <Text style={styles.tabletHeaderTitle}>{hermandad.name}</Text>
          ...
          /* MÓVIL: hero con imagen — igual que antes de cualquier cambio */
          <View style={styles.heroWrapper}>
            {headerSource ? (
              <View style={styles.heroContainer}>
                <Image
                  ...
                  resizeMode="cover"
                />
                <Image source={headerSource} style={styles.heroImg} resizeMode="contain" />
                <LinearGradient
            ...
            <View style={styles.nazarenoContainer}>
              <NazarenoIcon tunicColor={nazarenoColors.tunic} hoodColor={nazarenoColors.hood} capeColor={nazarenoColors.cape} size={44} />
            </View>
            <View style={styles.shieldContainer}>
              {(() => {
                const escudoSource = getEscudoSource(hermandad.shieldUrl)
                return escudoSource ? (
                  <Image source={escudoSource} style={styles.shield} resizeMode="contain" />
                ) : (
        ...
        {showAlert && incidencia && (
          <AlertBanner type={incidencia.type} title={incidencia.title} message={incidencia.message} />
        )}

        {/* En tablet: Horarios e Información en columnas; en móvil apilados */}
        {isTablet ? (
          <View style={styles.tabletRow}>
            <View style={styles.tabletCol}>{HorariosSection}</View>
            <View style={styles.tabletCol}>{InfoSection}</View>
          </View>
        ) : (
          <>
            {HorariosSection}
              ...
              hermandad.pasos.map((paso, idx) => (
                <View key={idx} style={styles.pasoCard}>
                  <View style={styles.pasoBadgeCorner}>
                    <LinearGradient colors={['#6a1b9a', '#8e24aa']} style={styles.pasoBadgeGradient}>
                  ...
                  {(paso.descripcion || paso.descripcion_imagen) && (
                    <View style={styles.pasoDescripcionContainer}>
                      <Text style={styles.pasoDescripcionText}>{paso.descripcion || paso.descripcion_imagen}</Text>
                    </View>
                  )}
            ...
            return (
              <View>
                ...
                {hermandad.itinerary.length > MAX_ITEMS && (
                  <TouchableOpacity onPress={() => setShowAll(!showAll)} style={styles.expandButton} activeOpacity={0.7}>
                    <Text style={styles.expandButtonText}>{showAll ? 'Ver menos' : `Ver más (${remaining} más)`}</Text>
        ...
        >
          ...
          <Text style={[styles.favoriteActionText, hermandad.isFavorite && styles.favoriteActionTextActive]}>
            {hermandad.isFavorite ? 'En Favoritas' : 'Añadir a Favoritas'}
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // ── MÓVIL: hero imagen (original, sin cambios) ──
  ...
  heroImg: { width: '100%', height: undefined, aspectRatio: 16 / 9 },
  ...
  // ── TABLET: header de gradiente con nombre ──
  tabletHeader: { width: '100%', paddingBottom: 24, overflow: 'hidden' },
  tabletHeaderTopBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 },
  tabletHeaderContent: { paddingHorizontal: 24, paddingTop: 16, paddingBottom: 12 },
  tabletHeaderTitle: { fontSize: 36, fontWeight: '800', color: colors.textLight, marginBottom: 12, textShadowColor: 'rgba(0,0,0,0.5)', textShadowOffset: { width: 0, height: 2 }, textShadowRadius: 4 },

  // ── Botones (mínimo 44x44 pt, requisito Apple HIG) ──
  circleBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center' },
  galleryPillBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: 'rgba(0,0,0,0.35)', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 22, minHeight: 44 },
  ...
  // ── Pasos detallados ──
  pasoCard: { marginBottom: 20, backgroundColor: '#ffffff', borderRadius: 16, overflow: 'hidden', elevation: 3, shadowColor: '#6a1b9a', shadowOpacity: 0.1, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, borderWidth: 1, borderColor: '#f0e6f8' },
  pasoBadgeCorner: { position: 'absolute', top: 0, right: 0, zIndex: 10 },
  pasoBadgeGradient: { width: 50, height: 50, borderBottomLeftRadius: 20, justifyContent: 'center', alignItems: 'center', paddingTop: 8, paddingRight: 8 },
  pasoBadgeNumber: { color: '#ffffff', fontSize: 18, fontWeight: '800', letterSpacing: 0.5 },
  pasoTitulo: { fontSize: 17, fontWeight: '700', color: '#2c1a40', lineHeight: 24, paddingTop: 18, paddingHorizontal: 16, paddingRight: 60, marginBottom: 12 },
  pasoDescripcionContainer: { backgroundColor: '#faf7ff', marginHorizontal: 12, marginBottom: 12, padding: 14, borderRadius: 12, borderLeftWidth: 3, borderLeftColor: '#d4af37' },
  pasoDescripcionText: { fontSize: 14, color: '#4a4a4a', lineHeight: 21, letterSpacing: 0.2 },
  pasoMetadatos: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, paddingHorizontal: 12, paddingBottom: 16 },
  metaChip: { flex: 1, minWidth: '45%', flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8f4ff', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 10, gap: 8, borderWidth: 1, borderColor: '#e8dbf5' },
  metaChipContent: { flex: 1 },
  metaChipLabel: { fontSize: 10, fontWeight: '700', color: '#8e24aa', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 2 },
  metaChipValue: { fontSize: 13, fontWeight: '600', color: '#2c1a40', lineHeight: 18 },
})
